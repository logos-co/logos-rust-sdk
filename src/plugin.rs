...
impl PluginProxy {
    ...
    pub fn call_sync<T: ToParam>(&self, method: &str, params: &[T]) -> Result<CallResult, LogosError> {
        self.call_sync_raw(method, params_to_json(params)?, None)
    }

    /// Call a plugin method synchronously with an explicit reply timeout,
    /// for methods known to outlive the transport's default.
    ...
    pub fn call_sync_with_timeout<T: ToParam>(
        ...
        self.call_sync_raw(method, params_to_json(params)?, Some(timeout))
    }
    ...
    pub fn call_sync_with_params(
        ...
        self.call_sync_raw(method, serde_json::to_string(params)?, None)
    }
    ...
    fn call_sync_raw(
        ...
        let method_c = CString::new(method)?;
        let params_json_c = CString::new(params_json)?;
        let timeout_ms = timeout.map_or(0, |t| {
            c_int::try_from(t.as_millis()).unwrap_or(c_int::MAX)
        });

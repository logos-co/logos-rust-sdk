{
  ...
  outputs = inputs@{ self, nixpkgs, logos-nix, logos-lidl, logos-module-builder, logos-logoscore-cli, logos-protocol }:
    ...
    in
    {
      ...
      # The same, over the Qt-free protocol runtime (liblogos_protocol_plain),
      # whose transports need no event loop in the caller.
      lib.callerBuildSupportPlain = nixpkgs.lib.genAttrs systems (system:
        let
          protocol = logos-module-builder.inputs.logos-protocol.packages.${system}.logos-protocol-plain;
        in
        {
          extraBuildInputs = [ protocol ];
          setupHook = ''
      ...
      # An app that embeds a runtime (feature `host`): liblogos, and the plain
      # protocol image liblogos itself links, both from liblogos' lib output.
      lib.hostBuildSupport = { liblogosLib }: {
        env.LOGOS_HOST_LIB_DIR = "${liblogosLib}/lib";
      ...
      # run the generator under nixpkgs.
      lib.mkClients = { system, lidls }:
        let
          pkgs = mkPkgs system;
          names = builtins.attrNames lidls;
        in
        pkgs.runCommand "logos-rust-clients" {
          nativeBuildInputs = [ self.packages.${system}.lidl-gen ];
        } ''
          mkdir -p $out
          ${nixpkgs.lib.concatMapStrings (name: ''
            logos-lidl-gen ${lidls.${name}} -o $out/${name}.rs
          '') names}
          {
            echo "// Generated by logos-rust-sdk lib.mkClients -- do not edit."
            ${nixpkgs.lib.concatMapStrings (name: ''
              echo "pub mod ${name};"
            '') names}
          } > $out/mod.rs
      ...
      # Fails when the committed clients differ from what the contracts generate.
      lib.clientsUpToDate = { system, lidls, committed }:
        let pkgs = mkPkgs system; in
        pkgs.runCommand "logos-rust-clients-up-to-date" { } ''
          if ! diff -ru ${self.lib.mkClients { inherit system lidls; }} ${committed}; then
            echo "committed clients are stale; regenerate them from lib.mkClients" >&2
            exit 1
          fi
          touch $out
        '';
      ...
      checks = nixpkgs.lib.genAttrs systems (system:
        ...
        in
        {
          ...
          module-impl-abi = pkgs.runCommand "rust-sdk-module-impl-abi" {
            ...
            # The scaffolds the FIXTURES actually compile. Regenerated by hand,
            # so they freeze the protocol they were emitted at while everything
            # above regenerates against the current one — the gap that shipped
            # at 0.3, 0.5, 0.6 and 0.9.
            COMMITTED_SCAFFOLDS = builtins.concatStringsSep " " [
              "provider=${./tests/provider/rust-lib/src/provider_gen.rs}"
              "caller=${./tests/caller/rust-lib/src/provider_gen.rs}"
            ];
          } ''
